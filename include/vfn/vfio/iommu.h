 ...
 * @iommu_be: &struct iommu back end
 ...
 */
int vfio_map_vaddr(struct vfio_iommu_be *iommu_be, void *vaddr, size_t len, uint64_t *iova);
 ...
 * Return: ``0`` on success, ``-1`` on error and sets ``errno``.
 */
int vfio_map_vaddr_ephemeral(struct vfio_iommu_be *iommu_be,
			     void *vaddr, size_t len, uint64_t *iova);

/**
 * vfio_unmap_ephemeral - free an ephemeral IO virtual address
 * @vfio: &struct vfio_container
